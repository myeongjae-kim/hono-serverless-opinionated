...
export class UserPersistenceAdapter implements UserCommandPort, UserQueryPort {
  ...
  async createUser(
    ...
    return await this.transactionTemplate.execute(
      ...
      async (tx) => {
        ...
        const insertedUser = await tx.select().from(user).where(
          eq(user.id, insertedId),
        ).limit(1);
  ...
  async findByLoginId(
    ...
    return await this.transactionTemplate.execute(sqlOptions, async (tx) => {
      const results = await tx.select().from(user).where(
  ...
  async findByUlid(ulid: string, sqlOptions: SqlOptions): Promise<User | null> {
    return await this.transactionTemplate.execute(sqlOptions, async (tx) => {
      const results = await tx.select().from(user).where(eq(user.ulid, ulid))
        .limit(1);
