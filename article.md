백엔드 애플리케이션을 만들 때 Java/Kotlin과 Spring Boot는 대부분 훌륭한 선택이
됩니다. Controller, Service, Repository로 이어지는 익숙한 계층 구조, IoC
...
예시 프로젝트: https://github.com/myeongjae-kim/hono-serverless-opinionated

위 프로젝트는 [Hono](https://hono.dev/), [Deno](https://deno.com/),
[Drizzle](https://orm.drizzle.team/), [Inversify](https://inversify.io/) 계열
IoC
컨테이너([inversify-typesafe-spring-like](https://www.npmjs.com/package/inversify-typesafe-spring-like))를
조합해서 Spring Boot에 익숙한 개발자도 납득할 수 있는 백엔드 구조를 TypeScript로
...
이 프로젝트에서는 위와 같은 상황의 팀의 생산성을 높이기 위해 다음 기준을
잡았습니다.

- 서버리스 배포를 기본값으로 두어 백엔드 인프라 설정 시간과 초기 운영 비용을
  줄입니다.
- 프론트엔드와 동일한 TypeScript를 사용해 언어, 타입, API schema를 하나의
  흐름으로 다룹니다.
- AI가 TypeScript 코드를 잘 작성하고, TypeScript 컴파일러와 linter가 빠른
  피드백을 주는 환경을 만듭니다.
- Docker 이미지를 기준으로 패키징해, 필요하면 ECS로 옮겨 전통적인 서버 기반
  백엔드 애플리케이션처럼 운영할 수 있게 합니다.

이런 상황에서 TypeScript는 프론트엔드와 백엔드의 좋은 공통 언어 역할을 할 수
있습니다. 프론트엔드는 이미 TypeScript를 사용하는 경우가 많고, 백엔드까지
TypeScript로 작성하면 도메인 타입, API 스키마, 검증 로직, 테스트 코드를 같은
언어로 다룰 수 있습니다. 팀의 컨텍스트 스위칭 비용도 줄어듭니다.
...
이 프로젝트가 지향하는 것은 작게 시작하되 크게 갈 수 있는 TypeScript 백엔드이자,
Spring Boot에 익숙하면서 인프라와 프론트엔드 개발도 수행해야 하는 팀의 생산성을
극대화하는 기술 스택입니다.

### 이 예시 프로젝트가 보여주는 것

예시
프로젝트([hono-serverless-opinionated](https://github.com/myeongjae-kim/hono-serverless-opinionated))는
Hono를 사용한 Deno 기반 백엔드 서버입니다. Hono로 구현한 단순한 hello world
프로젝트가 아니라 실제 백엔드 프로젝트에서 자주 필요한 요소를 하나의 opinionated
stack으로 묶어둔 예시입니다. 큰 그림은 다음 요청 흐름으로 이해할 수 있습니다.

```mermaid
flowchart TB
  Client["Client"] --> Controller["Hono Controller"]
...
Deno는 이미 상용 제품에 사용할 수 있을 정도로 성숙했습니다. Deno 2 계열에
들어오면서 npm 호환성이 더 자연스러워졌고, Deno 공식 블로그에서도
[Netlify와 Supabase edge functions, Slack platform 지원 사례](https://deno.com/blog/deno-in-2022)를
언급합니다. Deno 기반 런타임을 실제 제품 환경에서 사용하는 사례가 이미 여럿
있습니다.

물론 Node.js보다 생태계와 운영 사례가 작다는 점은 현실적인 리스크입니다. 따라서
상용 제품에서 Deno를 선택한다면 핵심 npm 라이브러리 호환성, APM과 로그 수집,
보안 스캔, 배포 환경, 장애 대응 루틴은 초기에 확인해야 합니다. 이 프로젝트에서
Deno를 선택한 이유는 TypeScript를 직접 실행할 수 있고, 개발 도구가 단순하며,
서버리스와 컨테이너 환경 양쪽에 올리기 쉬운 런타임이라는 점이 이 아키텍처의
목표와 잘 맞기 때문입니다.
...
Spring Boot 개발자에게 IoC 컨테이너는 애플리케이션의 의존성 방향을 지키고,
테스트 대역을 바꾸고, 환경별 구현체를 선택하는 중요한 도구입니다.

이 프로젝트는
[inversify-typesafe-spring-like](https://www.npmjs.com/package/inversify-typesafe-spring-like)를
사용해 Spring의 `@Autowired`와 비슷한 방식으로 의존성을 주입합니다.
`beanConfig.ts`에서 어떤 Port에 어떤 Adapter를 연결할지 정의하고,
`USE_MOCK_ADAPTER` 같은 환경 변수에 따라 mock adapter와 persistence adapter를
바꿀 수 있습니다.
...
아니지만, Spring Boot 개발자가 백엔드 코드를 작성할 때 기대하는 주요 역할은
TypeScript 스택 안에서도 상당 부분 대응시킬 수 있습니다.
...
```mermaid
flowchart TD
  Client["HTTP request"] --> Controller["Hono Controller<br/>app/api"]
  Controller --> InPort["In Port<br/>core/application/port/in"]
  InPort --> UseCase["Use Case<br/>core/application"]
  UseCase --> Domain["Domain<br/>core/domain"]
  UseCase --> OutPort["Out Port<br/>core/application/port/out"]
  OutPort --> Adapter["Adapter<br/>core/adapter/out"]
  Adapter --> Database["Drizzle / MySQL<br/>lib/db"]

  subgraph Inner["안쪽 계층"]
    Domain
    UseCase
    InPort
    OutPort
...
```mermaid
flowchart TB
  Image["Docker Image"] --> Lambda["AWS Lambda<br/>container runtime"]
  Image --> ECS["ECS<br/>container service"]

  Lambda --> LambdaEntry["Lambda entrypoint"]
  ECS --> ServerEntry["Deno.serve entrypoint"]

  LambdaEntry --> Hono["Same Hono App"]
  ServerEntry --> Hono

  Hono --> Core["core/<br/>Domain / Use Case / Port / Adapter"]
```
...
백엔드는 충분히 검토할 만한 선택지입니다. 특히 Spring Boot에 익숙하지만 인프라와
프론트엔드 개발까지 함께 수행해야 하는 팀이라면, 이 스택은 생산성을 높이면서도
확장 경로를 닫지 않는 현실적인 절충안이 될 수 있습니다.

## 부록

### 인프라 선택 기준

제품 초기에는 Vercel, PlanetScale, AWS Lambda 같은 관리형 서비스와 서버리스
조합이 설정 시간과 고정 운영 비용을 줄이는 데 유리합니다. 반대로 API 호출이
초기부터 많거나 백엔드를 독립적으로 확장해야 한다면 Hono API만 별도 Lambda 또는
컨테이너 서비스로 분리하는 편이 낫습니다.

제품이 성숙하고 트래픽이 상시적으로 커지면 ECS, Amazon Aurora, CloudWatch,
Grafana, OpenTelemetry 기반 관측성 도구처럼 운영 가시성을 전제로 한 조합을
검토할 수 있습니다. Fargate는 ECS 실행 방식 중 하나일 뿐이며, 더 많은 제어권이
필요하다면 ECS on EC2도 같은 아키텍처를 사용할 수 있습니다.

| 상황                                | 우선 고려할 조합                             |
| ----------------------------------- | -------------------------------------------- |
| 초기 MVP, 어드민, 사내 도구         | Vercel, PlanetScale, Lambda 같은 관리형 조합 |
| 초기부터 API 트래픽이 많은 서비스   | Hono 백엔드를 Lambda 또는 컨테이너로 분리    |
| 상시 트래픽이 높은 성숙 서비스      | ECS, Aurora, 관측성 도구 중심의 운영 환경    |
| 짧은 요청과 순간적인 spike 중심     | Lambda 컨테이너 이미지                       |
| 긴 요청, worker, 안정적 pool이 중요 | ECS 기반 상시 실행 환경                      |

실제 제품에 적용할 때는 서비스 지역, 데이터베이스 위치, 네트워크 latency, 관측성
요구사항, 팀의 운영 경험, 현재 가격 정책을 함께 확인해야 합니다.
