...
백엔드 애플리케이션을 만들 때 Java와 Spring Boot는 여전히 강력한 기본값입니다.
Controller, Service, Repository로 이어지는 익숙한 계층 구조, IoC 컨테이너, 검증,
인증, 예외 처리, 트랜잭션, OpenAPI 문서화, 테스트 도구까지 대부분의 백엔드
프로젝트에 필요한 요소가 잘 갖춰져 있습니다.
...
이 저장소는 그런 관점에서 만든 예시 프로젝트입니다. Hono, Deno, Drizzle,
Inversify 계열 IoC 컨테이너를 조합해서 Spring Boot에 익숙한 개발자도 납득할 수
있는 백엔드 구조를 TypeScript로 구성합니다. 동시에 Docker 이미지로 패키징해 AWS
Lambda 컨테이너 런타임에서 실행할 수 있고, 필요하면 ECS 같은 컨테이너 기반 상시
실행 환경으로 옮길 수 있게 설계합니다.
...
이 저장소는 Hono를 사용한 Deno 기반 백엔드 서버 예시입니다. 단순히 "Hono에서
...
이 프로젝트의 기술 스택은 "TypeScript로 백엔드를 만들 수 있다"는 이유만으로 고른
것이 아닙니다. 서버리스 환경, Spring Boot 개발자 경험, 확장 가능성, AI와의 협업,
운영 편의성을 함께 고려했습니다.
...
서버리스 환경에서는 런타임이 가볍고 예측 가능한 도구가 유리합니다. Drizzle은
무거운 query engine이나 별도 daemon을 요구하지 않기 때문에 Lambda 같은
환경에서도 부담이 작습니다.
...
예를 들어 Controller는 Hono route를 정의하고, 요청 schema와 응답을 OpenAPI에
등록합니다. 실제 비즈니스 로직은 Controller 안에 직접 작성하지 않고 IoC
컨테이너에서 Use Case를 꺼내 호출합니다.
...
API 통합 테스트는 pactum을 사용합니다. 테스트 시작 시 `setupIntTest()`로 base
URL을 설정하고, 각 테스트는 실제 endpoint에 요청을 보냅니다.
...
health check를 통과하면 통합 테스트를 실행합니다. Spring Boot의
`@SpringBootTest`나 MockMvc 테스트와 역할은 다르지만, API 서버가 실제로 요청을
처리하는지 검증한다는 점에서 비슷한 목적을 가집니다.
...
로컬 개발은 Deno task를 중심으로 구성되어 있습니다. 기본 흐름은 다음과 같습니다.
